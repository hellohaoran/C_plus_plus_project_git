...
int main(int argc, char *argv[])
{
    ...
    // 就绪队列;
    int listenSize = workerNum + 3; // 有socket描述符和子进程的管道加入监听
    int exitFlag = 0; // 不要退
    ...
    while (1)
    {
        ...
        for (int i = 0; i < readyNum; ++i)
        {
            if (readyArr[i].data.fd == sockFd)
            {
                ...
                for (int j = 0; j < workerNum; ++j)
                {
                    if (workerArr[j].status == FREE)
                    {
                        ...
                        sendFd(workerArr[j].pipfd, netFd,exitFlag); // 把链接移交给子进程
                        close(netFd);                      // 并不会把网络连接给关闭; 因为子进程已经连上
                        workerArr[j].status = BUSY;
                        break;
                    }
                }
            }else if(readyArr[i].data.fd == exitPip[0]){
                printf("process pool is going to exit!\n");
                exitFlag = 1; // 设为1 表示将要退出
                for(int j=0; j< workerNum; ++j){
                    // kill(workerArr[j].pid , SIGUSR1);
                    // 将
                    sendFd(workerArr[j].pipfd, 0 , exitFlag) ;
                }
               for(int j =0 ; j<workerNum;++j){
                    wait(NULL);
               }
            ...
            else
            {
                ...
                {
                    if (readyArr[i].data.fd == workerArr[j].pipfd)
                    { // 读就绪表示收到子进程发来的pid ; 收到子进程结束
                        pid_t pid;
